...
/**
 * Idempotent upsert of the supporting blog "Bookkeeping vs Accounting: What's the Difference for an Indian Business?".
 * Invoked every boot from docker/entrypoint.sh. Self-healing: fixes a
 * post hidden by a future published_at (UTC vs IST) and inserts if absent.
 */
class BookkeepingVsAccountingBlogSeeder extends Seeder
{
    public function run(): void
    {
        // Ensure the key_points column exists (Quick Answer box).
        if (! Schema::hasColumn('posts', 'key_points')) {
            Schema::table('posts', function ($t) {
                $t->text('key_points')->nullable();
            });
        }
