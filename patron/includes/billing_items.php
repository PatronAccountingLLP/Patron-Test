 ...
 * With Zoho Books integration for syncing items
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax'])) {
    ...
    if ($action === 'fetch_zoho_items') {
        ...
        while ($hasMore && $page <= 20) { // Max 20 pages to get all items
        ...
        // Mark which items are already mapped and check tax status
        foreach ($allItems as &$item) {
            $item['is_mapped'] = in_array($item['item_id'], $mappedIds);
            
            // Debug: Check what tax fields Zoho sends (remove after testing)
            // Possible tax fields: tax_id, is_taxable, tax_preference, tax_percentage, item_tax_preferences
            $item['debug_tax_fields'] = [
                'tax_id' => $item['tax_id'] ?? null,
                'is_taxable' => $item['is_taxable'] ?? null,
                'tax_preference' => $item['tax_preference'] ?? null,
                'tax_percentage' => $item['tax_percentage'] ?? null,
                'tax_name' => $item['tax_name'] ?? null,
            ];
            
            // Determine if taxable based on multiple possible fields
            $item['is_item_taxable'] = (
                !empty($item['tax_id']) || 
                (!empty($item['is_taxable']) && $item['is_taxable'] !== false && $item['is_taxable'] !== 'false') ||
                (isset($item['tax_preference']) && strtolower($item['tax_preference']) === 'taxable') ||
                (!empty($item['tax_percentage']) && $item['tax_percentage'] > 0) ||
                (!empty($item['tax_name']))
            );
        }
        
        echo json_encode(['success' => true, 'items' => $allItems, 'total' => count($allItems)]);
        exit;
    }
    
    // Bulk import Zoho items
    if ($action === 'bulk_import') {
        ...
        foreach ($items as $item) {
            ...
            // Check multiple tax fields from Zoho
            $isTaxable = (
                !empty($item['tax_id']) || 
                (!empty($item['is_taxable']) && $item['is_taxable'] !== false && $item['is_taxable'] !== 'false') ||
                (isset($item['tax_preference']) && strtolower($item['tax_preference'] ?? '') === 'taxable') ||
            ...
            $hsnSac = $item['hsn_or_sac'] ?? $item['hsn_sac_code'] ?? '';
            
            dbExecute("INSERT INTO billing_items (name, description, category_id, default_rate, hsn_sac_code, is_taxable, zoho_item_id, zoho_item_name) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                [$item['name'], $item['description'] ?? '', $categoryId ?: null, $rate, $hsnSac, $isTaxable, $item['item_id'], $item['name']]);
    ...
    // Link existing PATRON item to Zoho item
    if ($action === 'link_item') {
    ...
    if (!verifyCsrfToken($_POST['csrf_token'] ?? '')) {
        ...
        if ($action === 'edit') {
            ...
            if (empty($name)) {
                ...
                dbExecute("UPDATE billing_items SET name = ?, description = ?, category_id = ?, default_rate = ?, unit = ?, hsn_sac_code = ?, is_taxable = ?, is_active = ?, zoho_item_id = ?, zoho_item_name = ? WHERE id = ?",
                    [$name, $description, $categoryId ?: null, $defaultRate, $unit, $hsnSac, $isTaxable, $isActive, $zohoItemId ?: null, $zohoItemName ?: null, $id]);
        ...
        <table class="w-full">
            ...
            <tbody class="divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-4 py-3 text-center"><?= $item['is_taxable'] ? '<span class="px-2 py-1 text-xs font-medium rounded-full bg-blue-100 text-blue-700">18% GST</span>' : '<span class="px-2 py-1 text-xs font-medium rounded-full bg-gray-100 text-gray-600">Exempt</span>' ?></td>
        ...
        <form method="POST">
            ...
            <div class="p-6 space-y-4 max-h-96 overflow-y-auto">
                ...
                <div class="flex gap-6">
                    <label class="flex items-center gap-2"><input type="checkbox" name="is_taxable" id="editTaxable" class="rounded"><span class="text-sm">Taxable</span></label>
                ...
                <div class="border-t pt-4">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div><label class="form-label text-xs">Zoho Item ID</label><input type="text" name="zoho_item_id" id="editZohoId" class="form-input text-sm"></div>
                        <div><label class="form-label text-xs">Zoho Item Name</label><input type="text" name="zoho_item_name" id="editZohoName" class="form-input text-sm"></div>
                    </div>
                </div>
            </div>
            <div class="px-6 py-4 border-t border-gray-200 flex justify-end gap-3">
...
<div id="zohoSyncModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div class="bg-white rounded-lg shadow-xl w-full max-w-4xl mx-4 max-h-[90vh] flex flex-col">
        <div class="px-6 py-4 border-b border-gray-200 flex justify-between items-center">
            <div><h3 class="text-lg font-semibold">Sync Items from Zoho Books</h3><p class="text-sm text-gray-500">Import Zoho items to use in billing requests</p></div>
        ...
        <div class="px-6 py-4 border-t border-gray-200 flex justify-between items-center bg-gray-50">
            ...
            <div class="flex gap-3">
                ...
                <button onclick="importSelectedItems()" class="btn btn-primary" id="importBtn" disabled><i class="fas fa-download mr-2"></i>Import Selected</button>
            </div>
        </div>
    </div>
</div>
...
function editItem(item) {
    ...
    document.getElementById('editTaxable').checked = item.is_taxable == 1;
    document.getElementById('editActive').checked = item.is_active == 1;
    document.getElementById('editZohoId').value = item.zoho_item_id || '';
    document.getElementById('editZohoName').value = item.zoho_item_name || '';
...
function renderZohoItems() {
    ...
    tbody.innerHTML = filtered.map(item => `
        <tr class="hover:bg-gray-50 ${item.is_mapped ? 'bg-green-50' : ''}">
            <td class="px-4 py-2"><input type="checkbox" value="${item.item_id}" ${item.is_mapped ? 'disabled' : ''} ${selectedItems.has(item.item_id) ? 'checked' : ''} onchange="toggleItem('${item.item_id}')"></td>
            ...
            <td class="px-4 py-2 text-center" title="${item.debug_tax_fields ? JSON.stringify(item.debug_tax_fields) : ''}">${item.is_item_taxable ? '<span class="text-green-600 text-xs">Taxable</span>' : '<span class="text-gray-400 text-xs">Exempt</span>'}</td>
            <td class="px-4 py-2 text-center">${item.is_mapped ? '<span class="px-2 py-1 text-xs rounded-full bg-green-100 text-green-700">Imported</span>' : '<span class="px-2 py-1 text-xs rounded-full bg-gray-100 text-gray-600">New</span>'}</td>
        </tr>
    `).join('');
}
...
function toggleSelectAll(checkbox) {
    ...
    zohoItems.forEach(item => {
        if (item.name.toLowerCase().includes(search) && !item.is_mapped) {
...
async function importSelectedItems() {
    ...
    try {
        ...
        const data = await response.json();
        if (data.success) {
            alert('Imported ' + data.imported + ' items.' + (data.skipped > 0 ? ' ' + data.skipped + ' skipped.' : ''));
            location.reload();
        } else {
            alert('Error: ' + (data.error || 'Failed'));
            document.getElementById('importBtn').disabled = false;
            document.getElementById('importBtn').innerHTML = '<i class="fas fa-download mr-2"></i>Import Selected';
        }
    } catch (e) {
...
function searchZohoForLink(query) {
    ...
    searchTimeout = setTimeout(() => {
        ...
        container.innerHTML = results.length === 0 ? '<div class="p-3 text-gray-500 text-sm">No matching items</div>' : results.map(item => `<div class="p-3 hover:bg-gray-50 cursor-pointer border-b last:border-b-0" onclick="linkToZohoItem('${item.item_id}', '${escapeHtml(item.name).replace(/'/g, "\\'")}')"><div class="font-medium">${escapeHtml(item.name)}</div><div class="text-xs text-gray-500">ID: ${item.item_id} | ₹${parseFloat(item.rate || 0).toFixed(2)}</div></div>`).join('');
