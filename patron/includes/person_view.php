...
if ($action === 'add_dsc_history' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    dbExecute("INSERT INTO person_dsc_history (person_id, dsc_class, provider, token_number, issue_date, expiry_date, status, cost, vendor_name, notes, created_at)
        VALUES (?,?,?,?,?,?,?,?,?,?,NOW())", [
        $personId,
                ...
                <table class="w-full text-sm">
                    ...
                    <?php foreach ($dscHistory as $dh): ?>
                        <tr class="border-b hover:bg-gray-50">
                            ...
                            <td class="py-2 px-3"><?= htmlspecialchars($dh['provider'] ?? '') ?></td>
                            <td class="py-2 px-3 font-mono text-xs"><?= htmlspecialchars($dh['token_number'] ?? '') ?></td>
