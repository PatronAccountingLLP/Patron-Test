...
@php
    $tz      = 'Asia/Kolkata';
    $pageUrl = url()->current();
    $sAsset  = \Illuminate\Support\Str::title(trim($content->Asset ?? ''));
    $sRate   = isset($content->Rate) ? trim((string) $content->Rate) : '';
    $sRateDisplay = $sRate;
    if ($sRate !== '' && preg_match('/^\d+(?:\.\d+)?$/', $sRate)) { $sRateDisplay = $sRate.'%'; }
    $assetLbl = $sAsset !== '' ? $sAsset : 'this asset';
    $sCrumb   = $sAsset !== '' ? $sAsset : 'Depreciation Rate';
    ...
    // ---- Depreciation FAQ set (keep wording aligned with the visible FAQs in $content->content) ----
    $faqs = [
        ['q' => 'What is the income tax depreciation rate on '.$assetLbl.'?',
         'a' => $sRate !== ''
             ? 'The income tax depreciation rate on '.$assetLbl.' for FY 2025-26 is '.$sRateDisplay.', as prescribed under the Income Tax Act, 1961.'
             : 'The income tax depreciation rate applicable to '.$assetLbl.' for FY 2025-26 under the Income Tax Act, 1961 is shown in the table on this page.'],
        ['q' => 'How is depreciation calculated under the Income Tax Act?',
         'a' => 'Depreciation under Section 32 is computed on the written-down value (WDV) of a block of assets at the prescribed rate. If an asset is put to use for less than 180 days in the year, only half the rate is allowed for that year.'],
        ['q' => 'What is a block of assets?',
         'a' => 'A block of assets is a group of assets of the same class carrying the same depreciation rate (for example, buildings, or plant and machinery), on which depreciation is computed collectively rather than asset by asset.'],
        ['q' => 'Is income-tax depreciation the same as depreciation under the Companies Act?',
         'a' => 'No. Income-tax depreciation follows the prescribed rates and the block method under the Income Tax Act, 1961, whereas companies compute depreciation over an asset\'s useful life under Schedule II of the Companies Act, 2013.'],
        ['q' => 'When does the half-year (180-day) rule apply?',
         'a' => 'If an asset is acquired and put to use for less than 180 days during the financial year, only 50% of the normal depreciation rate is allowed in that year; the balance is effectively claimed in later years through the block.'],
    ];
    ...
    $schema = [
        ...
        '@graph'   => [
            ...
            [
                ...
                '@id'           => $pageUrl.'#faq',
                'url'           => $pageUrl,
                'name'          => $sCrumb.' — Income Tax Depreciation FAQs',
                'inLanguage'    => 'en-IN',
...
@php
    $asset = \Illuminate\Support\Str::title(trim($content->Asset ?? ''));

    // Headline the rate if the controller provides it; otherwise headline the asset. Both stay copyable.
    $rate = isset($content->Rate) ? trim((string) $content->Rate) : '';
    $rateDisplay = $rate;
    ...
    if ($rate !== '') {
        $featVal   = $rateDisplay;
        $featMono  = true;
        $featSub   = $asset !== '' ? $asset : 'Income Tax Act, 1961';
        $copyVal   = $rateDisplay;
        $copyLabel = 'Copy rate';
    } else {
        $featVal   = $asset;
        $featMono  = false;
        $featSub   = 'Income Tax Act, 1961';
        $copyVal   = $asset;
        $copyLabel = 'Copy asset name';
    }

    $crumb = $asset !== '' ? $asset : 'Depreciation Rate';
@endphp
        ...
        <div class="code-srch">
            <form class="code-srch__row" role="search" onsubmit="return false;">
                <input type="text" id="contentsearchInput" autocomplete="off"
                       placeholder="Search asset or depreciation rate…"
                       aria-label="Search asset or depreciation rate" />
...
<section style="background:#fff">
    <div class="layout">
        ...
        {{-- ---------------- LEFT: depreciation card (rate or asset + copy + context) ---------------- --}}
        <aside class="rail rail--left" aria-label="Depreciation quick reference">
            <div class="rail-inner">
                <div class="code-card">
                    <div class="code-card__top">
                        <div class="code-card__label"><span class="d"></span>Depreciation Rate</div>
                        @if($featVal !== '')
                            <div class="code-card__code{{ $featMono ? '' : ' code-card__code--text' }}" id="depRateValue">{{ $featVal }}</div>
                        @endif
                        <div class="code-card__sub">{{ $featSub }}</div>
                    </div>
                    ...
                    @if($featVal !== '')
                        <button type="button" class="copy-big js-copy"
                            ...
                            <span class="btn-label">{{ $copyLabel }}</span>
                        </button>
                    @endif

                    <div class="code-card__rows">
                        <dl>
                            <div class="pair"><dt>Basis</dt><dd>Block of assets</dd></div>
                            <div class="pair"><dt>Section</dt><dd>Sec 32, IT Act</dd></div>
                            <div class="pair"><dt>Period</dt><dd>FY 2025-26</dd></div>
                        </dl>
                    </div>
                </div>
            </div>
...
@push('meta-content')
    ...
    @php
        ...
        // ---- Standard depreciation FAQ set (keep wording aligned with the visible FAQs in $content->content) ----
        $faqs = [
            [
                'q' => 'What is the income tax depreciation rate on ' . $assetLbl . '?',
                'a' => $sRate !== ''
...
<style>
  /* ===================== PATRON Depreciation — design tokens ===================== */
  :root{
  ...
  *{box-sizing:border-box}
  ...
  /* ---------- LEFT RAIL: depreciation at-a-glance card ---------- */
  ...
  /* readable variant used when the headline is an asset name rather than a rate/code */
  .code-card__code--text{font-family:'DM Sans',-apple-system,system-ui,sans-serif;font-size:18px;letter-spacing:0;word-break:normal;line-height:1.3}
  ...
  /* internal-linking / related box — clean light pills (matches .nearby) */
  ...
  #searchResultsContainer .data-table{margin:0;border:0;border-radius:12px;min-width:420px;width:100%}
  ...
  @media (max-width:680px){
    ...
    .code-card__code--text{font-size:17px}
  }
...
<script>
  // FAQ accordion (for legacy .faq-item markup inside $content->content) + AJAX depreciation search
  $(document).ready(function () {
      ...
      let searchTimeout;
      const searchUrl = '/income-tax-depreciation-rate/search';
      ...
      function performSearch(searchTerm) {
          ...
          $.ajax({
              ...
              success: function (response) {
                  if (response.success && response.data && response.data.length > 0) {
                      ...
                      response.data.forEach(function (item) {
                          ...
                          if (asset || rate) {
                              resultsBody.append(
                                  ...
                                  '<td><a href="/income-tax-depreciation-rate/' + escapeHtml(String(slug)) + '">' + escapeHtml(String(rate)) + '</a></td>' +
                                  '</tr>'
                              );
                          }
                      });
                      ...
                      if (!searchTerm) {
                          ...
                          resultsBody.html('<tr><td colspan="2" style="text-align:center;padding:1.5rem;color:#666;">No results found for "' + escapeHtml(searchTerm) + '"</td></tr>');
              ...
              error: function (xhr) {
                  ...
                  resultsBody.html('<tr><td colspan="2" style="text-align:center;padding:2rem;color:#dc2626;">' + errorMsg + '</td></tr>');
                  resultsContainer.show();
              }
          });
      }
