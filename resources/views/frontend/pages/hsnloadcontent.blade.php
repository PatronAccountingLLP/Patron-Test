...
@php
    ...
    $sSlug   = $sParts[count($sParts) - 1] ?? '';
 
    $sCode = '';
    if (preg_match_all('/\d{2,8}/', $sSlug, $m) && !empty($m[0])) {
        usort($m[0], fn($a, $b) => strlen($b) - strlen($a));
        $sCode = $m[0][0];
    } elseif (preg_match('/\d{2,8}/', trim(strip_tags($content->h1 ?? '')), $mm)) {
        $sCode = $mm[0];
    }
    $sCrumb = $sCode !== '' ? 'HSN Code '.$sCode : trim(strip_tags($content->h1 ?? 'HSN Code'));
 
   
 
    // ---- HSN FAQ set (keep wording aligned with the visible FAQs in $content->content) ----
    if ($sCode !== '') {
        $faqs = [
            ['q' => 'What is HSN code '.$sCode.'?',
             'a' => 'HSN code '.$sCode.' is a classification under the Harmonized System of Nomenclature used to identify goods for GST in India. The goods description and applicable GST rate are shown in the details on this page.'],
            ['q' => 'What is the GST rate for HSN code '.$sCode.'?',
             'a' => 'The GST rate applicable to HSN '.$sCode.' is shown in the details above. GST rates are revised from time to time through CBIC notifications, so always confirm the current rate before raising an invoice.'],
            ['q' => 'How many digits does an HSN code have?',
             'a' => 'Under Indian GST, HSN codes are 2, 4, 6 or 8 digits. Businesses quote 4, 6 or 8 digits on invoices depending on turnover; code '.$sCode.' follows this same structure.'],
            ['q' => 'What is the structure of HSN code '.$sCode.'?',
             'a' => 'An HSN code breaks down into a 2-digit Chapter, 4-digit Heading and 6-digit Sub-heading, extending to an 8-digit tariff item in India. Code '.$sCode.' sits within this hierarchy.'],
            ['q' => 'Where is the HSN code used?',
             'a' => 'HSN codes are quoted on GST invoices and in GSTR-1 returns, and are used to classify goods for GST rate determination and customs.'],
        ];
    } else {
        $faqs = [
            ['q' => 'What is an HSN code?',
             'a' => 'HSN (Harmonized System of Nomenclature) is an internationally adopted system of codes used to classify goods. In India it is used to determine GST rates and to report goods on GST returns.'],
            ['q' => 'How many digits does an HSN code have?',
             'a' => 'Under Indian GST, HSN codes are 2, 4, 6 or 8 digits. Businesses quote 4, 6 or 8 digits on invoices depending on their turnover.'],
            ['q' => 'What is the structure of an HSN code?',
             'a' => 'An HSN code breaks down into a 2-digit Chapter, 4-digit Heading and 6-digit Sub-heading, extending to an 8-digit tariff item in India.'],
            ['q' => 'Where is the HSN code used?',
             'a' => 'HSN codes are quoted on GST invoices and in GSTR-1 returns, and are used to classify goods for GST rate determination and customs.'],
        ];
    }
...
@php
    // ---- HSN code derived from the URL slug (or the <h1> text as a fallback) ----
    $url   = url()->current();
    $parts = explode('/', trim($url, '/'));
    $slug  = $parts[count($parts) - 1] ?? '';

    $code = '';
    if (preg_match_all('/\d{2,8}/', $slug, $m) && !empty($m[0])) {
        usort($m[0], fn($a, $b) => strlen($b) - strlen($a));   // prefer the longest digit run
        $code = $m[0][0];
    } elseif (preg_match('/\d{2,8}/', trim(strip_tags($content->h1 ?? '')), $mm)) {
        $code = $mm[0];
    }

    // ---- HSN hierarchy (Chapter 2 · Heading 4 · Sub-heading 6 · Tariff item 8) ----
    $len           = strlen($code);
    $rowChapter    = $len > 2 ? substr($code, 0, 2) : null;
    $rowHeading    = $len > 4 ? substr($code, 0, 4) : null;
    $rowSubheading = $len > 6 ? substr($code, 0, 6) : null;
    $level = match(true) {
        $len >= 8 => 'Tariff item (8-digit)',
        $len == 6 => 'Sub-heading (6-digit)',
        $len == 4 => 'Heading (4-digit)',
        $len == 2 => 'Chapter (2-digit)',
        $len > 0  => $len . '-digit code',
        default   => null,
    };
...
<section style="background:#fff">
    <div class="layout">
        ...
        {{-- ---------------- LEFT: HSN code card (code + copy + derived hierarchy) ---------------- --}}
        <aside class="rail rail--left" aria-label="HSN quick reference">
            <div class="rail-inner">
                <div class="code-card">
                    <div class="code-card__top">
                        <div class="code-card__label"><span class="d"></span>HSN Code</div>
                        @if($code !== '')
                            <div class="code-card__code" id="hsnValue">{{ $code }}</div>
                        @endif
                        <div class="code-card__sub">Harmonized System of Nomenclature</div>
                    </div>

                    @if($code !== '')
                        <button type="button" class="copy-big js-copy"
                                data-copy="{{ $code }}"
                                aria-label="Copy HSN code {{ $code }}">
                            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                <rect x="9" y="9" width="11" height="11" rx="2"/>
                                <path d="M5 15V5a2 2 0 0 1 2-2h10"/>
                            </svg>
                            <span class="btn-label">Copy HSN Code</span>
                        </button>
                    @endif
                    ...
                    <div class="code-card__rows">
                        <dl>
                            @if(!empty($rowChapter))
                                <div class="pair"><dt>Chapter</dt><dd>{{ $rowChapter }}</dd></div>
                            @endif
                            @if(!empty($rowHeading))
                                <div class="pair"><dt>Heading</dt><dd>{{ $rowHeading }}</dd></div>
                            @endif
                            @if(!empty($rowSubheading))
                                <div class="pair"><dt>Sub-heading</dt><dd>{{ $rowSubheading }}</dd></div>
                            @endif
                            @if(!empty($level))
                                <div class="pair"><dt>Type</dt><dd>{{ $level }}</dd></div>
                            @endif
                            <div class="pair"><dt>Classification</dt><dd>HSN (GST)</dd></div>
                        </dl>
                    </div>
                </div>
            </div>
        ...
        <main class="content">
            <div id="originalContent">
                {{-- $body is $content->content with the dead codes in the stored
                     "HSN Codes under Chapter NN" table unlinked - the table and its
                     rows stay, only the anchors on codes that 404 or 410 are dropped.
                     See DynamicController::unlinkDeadHsnLinks. Falls back to the raw
                     body if the controller did not supply one. --}}
...
@push('meta-content')
    ...
    @php
        ...
        // ---- Standard HSN FAQ set (keep wording aligned with the visible FAQs in $content->content) ----
        if ($sCode !== '') {
            $faqs = [
                [
                    'q' => 'What is HSN code ' . $sCode . '?',
...
<style>
  ...
  *{box-sizing:border-box}
  ...
  /* ---------- LEFT RAIL: HSN at-a-glance card ---------- */
...
<script>
  // FAQ accordion (for legacy .faq-item markup inside $content->content) + AJAX HSN search
  $(document).ready(function () {
      ...
      let searchTimeout;
      const searchUrl = '/hsn-code/search';
              ...
              success: function (response) {
                  if (response.success && response.data && response.data.length > 0) {
                      ...
                      response.data.forEach(function (item) {
                          ...
                          const description = item.HSNDescription || '';
                          const slug        = item.Slug || '';
                          if (chapter || hsnCode || description) {
                              resultsBody.append(
                                  '<tr>' +
                                  '<td>' + escapeHtml(String(chapter)) + '</td>' +
                                  '<td><a href="/hsn-code/' + escapeHtml(String(slug)) + '">' + escapeHtml(String(hsnCode)) + '</a></td>' +
