...
@php
    $tz      = 'Asia/Kolkata';
    $pageUrl = url()->current();
    $sParts  = explode('/', trim($pageUrl, '/'));
    $sDesc   = ucwords(str_replace('-', ' ', $sParts[count($sParts) - 2] ?? ''));
    $sCode   = $sParts[count($sParts) - 1] ?? '';
    ...
    // ---- NIC FAQ set (keep wording aligned with the visible FAQs in $content->content_html) ----
    $faqs = [
        ['q' => 'What is the NIC code for '.$sDesc.'?',
         'a' => 'The NIC code for '.$sDesc.' is '.$sCode.', as classified under the National Industrial Classification (NIC 2008).'],
        ['q' => 'What does NIC code '.$sCode.' represent?',
         'a' => 'NIC code '.$sCode.' represents the business activity "'.$sDesc.'" under the National Industrial Classification (NIC 2008) maintained by the Ministry of Statistics and Programme Implementation.'],
        ['q' => 'Why is a NIC code required?',
         'a' => 'A NIC code classifies the economic activity of a business. It is mandatory when incorporating a company or LLP with the MCA, and is also referenced for Udyam/MSME, IEC and other registrations.'],
        ['q' => 'What is the structure of a NIC code?',
         'a' => 'NIC 2008 codes are hierarchical — a 2-digit Division, 3-digit Group, 4-digit Class and 5-digit Sub-class. Code '.$sCode.' sits within this hierarchy.'],
        ['q' => 'Where is the NIC code used during company registration?',
         'a' => 'It is entered in the SPICe+ (INC-32) incorporation form and stated in the Memorandum of Association (MoA), or in the FiLLiP form and LLP agreement for an LLP.'],
    ];
    ...
    $schema = [
        ...
        '@graph'   => [
            ...
            [
                ...
                '@id'           => $pageUrl.'#faq',
                'url'           => $pageUrl,
                'name'          => 'NIC Code '.$sCode.' — '.$sDesc.' — FAQs',
                'inLanguage'    => 'en-IN',
...
@php
    // ---- Values derived from the URL: /nic-code/{description-slug}/{code} ----
    $url   = url()->current();
    $parts = explode('/', trim($url, '/'));

    $description = ucwords(str_replace('-', ' ', $parts[count($parts) - 2] ?? ''));
    $code        = $parts[count($parts) - 1] ?? '';
    $finalTitle  = $description . ' - ' . $code;

    // ---- NIC 2008 hierarchy, derived from the (numeric) code ----
    $nicDigits = preg_replace('/\D/', '', $code);
    $len       = strlen($nicDigits);

    $div2        = $len >= 2 ? substr($nicDigits, 0, 2) : null;   // 2-digit division (for section lookup)
    $rowDivision = $len >  2 ? substr($nicDigits, 0, 2) : null;   // show as ancestor only if code is longer
    $rowGroup    = $len >  3 ? substr($nicDigits, 0, 3) : null;
    $rowClass    = $len >  4 ? substr($nicDigits, 0, 4) : null;
    ...
    if ($div2 !== null) {
        $d = (int) $div2;
        $section = match(true) {
            $d >= 1  && $d <= 3   => 'A — Agriculture & allied',
            $d >= 5  && $d <= 9   => 'B — Mining & quarrying',
            $d >= 10 && $d <= 33  => 'C — Manufacturing',
            $d === 35             => 'D — Electricity & gas',
            $d >= 36 && $d <= 39  => 'E — Water supply & waste',
            $d >= 41 && $d <= 43  => 'F — Construction',
            $d >= 45 && $d <= 47  => 'G — Wholesale & retail trade',
            $d >= 49 && $d <= 53  => 'H — Transport & storage',
            $d >= 55 && $d <= 56  => 'I — Accommodation & food',
            $d >= 58 && $d <= 63  => 'J — Information & communication',
            $d >= 64 && $d <= 66  => 'K — Financial & insurance',
            $d === 68             => 'L — Real estate',
            $d >= 69 && $d <= 75  => 'M — Professional & technical',
            $d >= 77 && $d <= 82  => 'N — Administrative & support',
            $d === 84             => 'O — Public administration',
            $d === 85             => 'P — Education',
            $d >= 86 && $d <= 88  => 'Q — Human health & social work',
            $d >= 90 && $d <= 93  => 'R — Arts & recreation',
            $d >= 94 && $d <= 96  => 'S — Other services',
            $d >= 97 && $d <= 98  => 'T — Household employers',
            $d === 99             => 'U — Extraterritorial bodies',
            default               => null,
        };
    }
@endphp
        ...
        <div class="code-srch">
            <form class="code-srch__row" role="search" onsubmit="return false;">
                <input type="text" id="contentsearchInput" autocomplete="off"
                       placeholder="Search NIC code or description…"
                       aria-label="Search NIC code or description" />
...
<section style="background:#fff">
    <div class="layout">
        ...
        {{-- ---------------- LEFT: NIC code card (code + copy + derived hierarchy) ---------------- --}}
        <aside class="rail rail--left" aria-label="NIC quick reference">
            <div class="rail-inner">
                <div class="code-card">
                    <div class="code-card__top">
                        <div class="code-card__label"><span class="d"></span>NIC Code</div>
                        <div class="code-card__code" id="nicValue">{{ $code }}</div>
                        <div class="code-card__sub">{{ $description }}</div>
                    </div>

                    <button type="button" class="copy-big js-copy"
                            data-copy="{{ $code }}"
                            aria-label="Copy NIC code {{ $code }}">
                        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                            <rect x="9" y="9" width="11" height="11" rx="2"/>
                            <path d="M5 15V5a2 2 0 0 1 2-2h10"/>
                        </svg>
                        <span class="btn-label">Copy NIC Code</span>
                    </button>

                    <div class="code-card__rows">
                        <dl>
                            @if(!empty($section))
                                <div class="pair"><dt>Section</dt><dd>{{ $section }}</dd></div>
                            @endif
                            @if(!empty($rowDivision))
                                <div class="pair"><dt>Division</dt><dd>{{ $rowDivision }}</dd></div>
                            @endif
                            @if(!empty($rowGroup))
                                <div class="pair"><dt>Group</dt><dd>{{ $rowGroup }}</dd></div>
                            @endif
                            @if(!empty($rowClass))
                                <div class="pair"><dt>Class</dt><dd>{{ $rowClass }}</dd></div>
                            @endif
                            @if(!empty($level))
                                <div class="pair"><dt>Type</dt><dd>{{ $level }}</dd></div>
                            @endif
                            <div class="pair"><dt>Classification</dt><dd>NIC 2008</dd></div>
                        </dl>
                    </div>
                </div>
            </div>
...
@push('meta-content')
    {!! $content->meta_title_html !!}
    {!! $content->canonical_html !!}
    ...
    @php
        // Re-derive locally so this block is self-contained (independent of the content section).
        $tz       = 'Asia/Kolkata';
        $pageUrl  = url()->current();
        $sParts   = explode('/', trim($pageUrl, '/'));
        $sDesc    = ucwords(str_replace('-', ' ', $sParts[count($sParts) - 2] ?? ''));
        ...
        // ---- Standard NIC FAQ set (keep wording aligned with the visible FAQs in $content->content_html) ----
        $faqs = [
            [
                'q' => 'What is the NIC code for ' . $sDesc . '?',
                'a' => 'The NIC code for ' . $sDesc . ' is ' . $sCode . ', as classified under the National Industrial Classification (NIC 2008).',
...
<style>
  ...
  :root{
    ...
    --header-h:72px;
    --maxw:1240px;
    --primary-color:#14365F; /* used by the dynamic $content->content_html blob */
  }
  *{box-sizing:border-box}
  ...
  /* ---------- LEFT RAIL: NIC at-a-glance card ---------- */
  ...
  /* ===================== LEGACY content + search (keeps existing $content->content_html rendering) ===================== */
  ...
  /* DESCRIPTION column (3rd) stays left-aligned */
...
<script>
  // FAQ accordion (for legacy .faq-item markup inside $content->content_html) + AJAX NIC search
  $(document).ready(function () {
      ...
      let searchTimeout;
      const searchUrl = '/nic-code/search';
              ...
              success: function (response) {
                  if (response.success && response.data && response.data.length > 0) {
                      ...
                      response.data.forEach(function (item) {
                          const classVal    = item.Class || item.class || '';
                          const slug        = item.slug || '';
                          const subclassVal = item.SubClass || item.subclass || '';
                          const descVal     = item.FullDescription || item.fullDescription || item.full_description || '';
                          if (classVal || subclassVal || descVal) {
                              resultsBody.append(
                                  '<tr>' +
                                  '<td>' + escapeHtml(String(classVal)) + '</td>' +
                                  '<td><a href="/nic-code/' + escapeHtml(String(slug)) + '/' + escapeHtml(String(subclassVal)) + '">' + escapeHtml(String(subclassVal)) + '</a></td>' +
                                  '<td>' + escapeHtml(String(descVal)) + '</td>' +
                                  '</tr>'
                              );
                          }
                      });
